/*
 * ring_attention_staged_gpu.cu
 * Ring Attention Skeleton — CUDA + MPI (staged) + GPU attention
 *
 * Compile:
 *   nvcc -o ring_attention_staged_gpu ring_attention_staged_gpu.cu \
 ...
 *   -np 2 ./ring_attention_staged_gpu
 */
#include<stdio.h>
#include<stdlib.h>
#include<math.h>
...
int main(int argc, char **argv){
    ...
    CHECK_CUDA(cudaDeviceSynchronize());
        ...
        // for (int d = 0; d < 5; d++) {
        //     // printf("[Rank %d] row %d: gpu_m=%.6f cpu_m=%.6f\n",
        //     //     rank, i, h_m_from_gpu[i], m_local[i]);
        //     // printf("[Rank %d] row %d: gpu_l=%.6f cpu_l=%.6f\n",
        //     //    rank, i, h_l_from_gpu[i], l_local[i]);
        //     printf("[Rank %d] out[0,%d]: gpu=%.6f cpu=%.6f\n",
        //        rank, d, h_acc_from_gpu[d], acc_local[d]);
        // }
